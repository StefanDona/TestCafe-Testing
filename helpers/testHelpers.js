...
class TestHelpers {
    ...
    async returnRandomSelectorFromMultipleSelectors(selector, startingIndex) {
        var randomNumber = await this.returnRandomNumberForMultipleSelector(selector, startingIndex);
    ...
    async selectRandomOptionFromDropdown(select) {
        ...
        var randomSelector = await this.returnRandomSelectorFromMultipleSelectors(options);

        if (await (randomSelector.value) === 'Select one' || await randomSelector.value === '') {
            randomSelector = await this.returnRandomSelectorFromMultipleSelectors(options);
        }
